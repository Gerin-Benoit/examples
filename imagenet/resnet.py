...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, norm=nn.BatchNorm2d, c=0, device='cpu', mod=False,
                 fc_sn=False):
        ...
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # difference with CIFAR10/100 script
...
def wrapper_spectral_norm(layer, shape, kernel_size, c=1):
    if c == 0:
        return layer
    if c > 0:
