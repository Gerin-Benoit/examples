...
"""
import torch
from torch.nn.functional import normalize, conv_transpose2d, conv2d, conv_transpose3d, conv3d
...
class SpectralNormConv(object):
    ...
    def compute_weight(self, module, do_power_iteration):
            ...
            with torch.no_grad():
                ...
                for _ in range(self.n_power_iterations):
                    if len(self.input_dim) == 4:  # 2D conv
                        ...
                        v_s = conv_transpose3d(u.view(self.out_shape), weight, stride=stride,
                                               padding=padding, output_padding=0)
                        # Note: out flag for in-place changes
                        v = normalize(v_s.view(-1), dim=0, eps=self.eps, out=v)

                        u_s = conv3d(v.view(self.input_dim), weight, stride=stride, padding=padding,
                                     bias=None)
                    else:
                        print('WARNING: spectral norm cannot be applied !')
                        exit(-1)
        ...
        if len(self.input_dim) == 4:  # 2D conv
            ...
            weight_v = conv3d(v.view(self.input_dim), weight, stride=stride, padding=padding,
                              bias=None)
    ...
    def apply(module, coeff, input_dim, name, n_power_iterations, eps):
        ...
        with torch.no_grad():
            res = 1
            for ele in input_dim:
                res *= ele
            num_input_dim = res  # input_dim[0] * input_dim[1] * input_dim[2] * input_dim[3]
            ...
            if len(input_dim) == 4:  # 2D conv
                u = conv2d(v.view(input_dim), weight, stride=stride, padding=padding,
                           bias=None)
            elif len(input_dim) == 5:  # 3D conv
                u = conv3d(v.view(input_dim), weight, stride=stride, padding=padding,
                           bias=None)
            fn.out_shape = u.shape
            res = 1
            for ele in fn.out_shape:
                res *= ele
            num_output_dim = res  # fn.out_shape[0] * fn.out_shape[1] * fn.out_shape[2] * fn.out_shape[3]
...
def spectral_norm_conv(module, coeff, input_dim, name='weight', n_power_iterations=1, eps=1e-12):
    ...
    input_dim_4d = (1,) + input_dim
    SpectralNormConv.apply(module, coeff, input_dim_4d, name, n_power_iterations, eps)
    return module


def remove_spectral_norm_conv(module, name: str = 'weight'):
